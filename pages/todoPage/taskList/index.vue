    ...
    <div id="homePage">
      ...
      <div class="control">
        ...
        >
          ...
          <b-form-input
            v-model="todoValue"
            placeholder="Enter your task" :state="inputTodoState"
          ></b-form-input>
          <p>Description</p>
          <b-form-textarea
            id="textarea-default"
            placeholder="Enter task's description...."
            v-model="descriptionValue" :state="inputDescState"
          ></b-form-textarea>
        </b-modal>
        <b-form-checkbox
          v-model="checked"
      ...
      >
        ...
        <template #cell(todo)="data">
          <p @click="data.toggleDetails" :class="{ active: data.item.status }" class="todoContent" v-b-tooltip.hover title="Click to see details content">
            {{ data.value }}
          </p>
        </template>
        <template #cell(status)="data1">
          <b-button :variant="changeBgStatus(data1.item.status)" @click="onStatus(data1.index)">{{
            data1.value
          }}</b-button>
        </template>
      ...
      <b-modal :id="infoModal.id" :title="infoModal.title" @hide="resetInfoModal" :ok-disabled="!validInput" @ok="updateTodo">
          <p>Task</p>
          <b-form-input
            v-model="todoValue"  
            placeholder="Enter your task" :state="inputTodoState"
          ...
          <b-form-textarea
            id="textarea-default"
            placeholder="Enter task's description...." 
            v-model="descriptionValue" @input="updateDescription(descriptionValue)" :state="inputDescState"
  ...
  components: { headerDefault },
    ...
    computed:{
      ...
      inputDescState(){
        ...
        else if(this.descriptionValue.length > 150)
      ...
      validInput(){
        return this.isValidDes === true && this.isValidTodo===true
      }
    },
    methods: {
      onAdd() {
